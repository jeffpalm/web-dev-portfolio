...
<nav class="nav-root" aria-label="main navigation" out:fade>
  <div class="nav-toolbar">
    {#each navLinks as { buttonText, link }, index}
      <NavButton --delay={index} buttonText={buttonText} onClick={handleNavClick(link)}
                 isActive={activePage === link} />
    {/each}
  </div>
  <button type="button" class={`nav-logo${rotateTree ? ' palmy-rotate' : ''}`} onclick={handlePalmytreeClick}
          aria-roledescription="button" aria-label="change color" in:fly={{x: 100, delay: 1000}}>
