  ...
  return (
    <FullPage name='skills' id='skills-page'>
      <MotionGrid container>
        <MotionGrid item container direction='column' alignItems='center' xs>
          <SkillLegend />

          {categories.map((category) => (
            <React.Fragment key={category}>
              <MotionTypo variant='h4' color='textPrimary' align='center'>
                {category}
              </MotionTypo>
              {skills
                ...
                .map((skill, i) => (
                  ...
                  >
                    <div className={classes.skillContainer}>
                      ...
                      <SkillStars starCount={skill.level} name={skill.title} />
                    </div>
                    {skill.gitHubName && gitHubStats.lang[skill.gitHubName] && (
                      <motion.div className={classes.gitHubBar}>
                        <MotionTypo
                          variant='body1'
                          color='textSecondary'
                          display='inline'
                        >
                          GitHub Additions: {gitHubStats.lang[skill.gitHubName]}
                        </MotionTypo>
                      </motion.div>
                    )}
                  </motion.div>
          ...
          {gitHubStats.pkgs.length !== 0 && (
            <Suspense fallback={<div>Loading...</div>}>
              <MaterialTable
                ...
                data={gitHubStats.pkgs.sort((a, b) => b.count - a.count)}
                options={{
                  search: false,
                  showTitle: false,
                  draggable: false,
